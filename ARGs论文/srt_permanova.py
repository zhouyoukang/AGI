...
for d1 in ['Mild','Moderate','Severe']:
    for d2 in ['Mild','Moderate','Severe']:
        if d1 < d2:
            ...
            f, p, r2 = permanova(sub_D, sub_groups, n_perm=999)
            sig = '***' if p<0.001 else '**' if p<0.01 else '*' if p<0.05 else 'ns'
            print(f"  {d1} vs {d2}: F={f:.4f}, P={p:.4f} {sig}, R2={r2:.4f}")
            pairwise_results.append({'Group1': d1, 'Group2': d2, 'F': f, 'P': p, 'R2': r2, 'sig': sig})
...
pw_df.to_csv(os.path.join(OUT_DIR, 'PERMANOVA_pairwise.csv'), index=False)
print(f"Pairwise results saved to PERMANOVA_pairwise.csv")
