...
ARG丰度以每个样本的注释ORF数量表示，相对丰度为各ARG类别ORF数占该样本总ARG ORF数的比例。采用Kruskal-Wallis检验比较不同干旱梯度间的ARG丰度和多样性差异，显著差异的组间进一步采用Mann-Whitney U检验进行两两比较。逐类别检验后采用Benjamini-Hochberg法进行FDR校正(q<0.05为显著)。基于Bray-Curtis距离进行主坐标分析(PCoA)和置换多元方差分析(PERMANOVA, 999次置换)评估干旱梯度、植物种和土壤类型对ARG群落组成的解释力。Spearman相关分析评估环境因子与ARG丰度的关联。所有统计分析使用Python 3.11 (scipy, scikit-learn, statsmodels, pandas)完成。
...
尽管ARG总量和多样性在干旱梯度间保持稳定，逐类别Kruskal-Wallis检验揭示了79个ARG类别中有18个(22.8%)在干旱梯度上发生了显著的相对丰度变化(P < 0.05)(图9)。经Benjamini-Hochberg FDR校正后，仍有8个ARG类别保持显著(q < 0.05)，包括tetracenomycin_c(q < 0.001)、puromycin(q=0.002)、tunicamycin(q=0.008)、qa_compound(q=0.016)、AF(q=0.018)、lincomycin(q=0.022)、streptomycin(q=0.024)和tigecycline(q=0.024)。
...
PERMANOVA分析(基于Bray-Curtis距离, 999次置换)揭示了ARG群落组成受多个因子的共同驱动(表2)。土壤类型(根际/非根际)解释了最大比例的ARG组成变异(R²=0.719, F=104.87, P=0.001)，其次为植物种(R²=0.586, F=28.27, P=0.003)和干旱梯度(R²=0.562, F=25.65, P=0.021)。

两两比较显示，轻微干旱与重度干旱之间的ARG组成差异显著(P=0.045)，中度干旱与重度干旱之间存在边际显著差异(P=0.069)，而轻微与中度干旱间差异不显著(P=0.125)，提示重度干旱是驱动ARG组成分异的主要阈值。
...
PERMANOVA分析揭示了一个层次化的ARG调控格局：根际/非根际微环境差异(R²=71.9%, P=0.001)>植物种(R²=58.6%, P=0.003)>干旱梯度(R²=56.2%, P=0.021)。根际效应的主导地位可能源于植物根系分泌物对微生物群落的强烈选择作用——根际土壤中特定微生物功能群的富集直接影响了ARG组成。
