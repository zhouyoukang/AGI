...
> **论文阶段**：已提交v19稿（2026-04-03），待导师审阅反馈
...
| **最新提交** | `稿/ER_Submission_v19.0_20260403.zip` |
...
├── figures/                  ← 图表产出(可重新生成)
│   ├── Fig1-10              ← 10张正文图(PCoA含方差解释%)
│   ├── FigP1-P8             ← 8张补充图(α多样性/网络/风险分类等)
│   ├── per_class_stats.csv  ← 79个ARG的KW检验 + FDR q值
│   ├── Table2_SigARGs_summary.csv  ← 18个显著ARG汇总 + FDR
...
### 2. 22.8%的ARG类别显著重组（18/79 P<0.05, FDR后8/79 q<0.05）
...
- [ ] 确认根际/非根际划分（1-3/4-6）
- [x] ~~运行 `python srt_make_docx.py` 重新生成含所有图的Word~~ — PERMANOVA值已改为动态读取CSV
...
- [x] ~~多重比较校正~~ — 已实现BH FDR校正，FDR后8/79仍显著，q值已写入Table2
