...
# --- Fig 2: PCoA (classical multidimensional scaling with eigenvalue-based variance) ---
print("  Fig2: PCoA...")
n = len(rel_abundance)
if n >= 3:
    ...
    # Classical PCoA via eigendecomposition (Gower 1966)
    D2 = dist_mat ** 2
    n_pcoa = D2.shape[0]
    H = np.eye(n_pcoa) - np.ones((n_pcoa, n_pcoa)) / n_pcoa  # centering matrix
    B = -0.5 * H @ D2 @ H
    ...
    var_explained = pos_eigvals / pos_eigvals.sum() * 100
    # Project onto first 2 axes
    coords = eigvecs[:, :2] * np.sqrt(np.maximum(eigvals[:2], 0))
    pc1_var = var_explained[0] if len(var_explained) > 0 else 0
    pc2_var = var_explained[1] if len(var_explained) > 1 else 0
    ...
    ax.set_xlabel(f'PCoA1 ({pc1_var:.1f}%)')
    ax.set_ylabel(f'PCoA2 ({pc2_var:.1f}%)')
    ...
    plt.close()
    print(f"    -> Fig2 saved (PC1={pc1_var:.1f}%, PC2={pc2_var:.1f}%)")
