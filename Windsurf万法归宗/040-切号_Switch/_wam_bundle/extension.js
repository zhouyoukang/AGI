// WAM v6.0 — 重置时间感知 · 智能切号 · 自使用追踪 · 万法归宗
// 道冲，而用之或不盈。渊兮，似万物之宗。
...
function log(msg) {
  ...
  try { fs.mkdirSync(WAM_DIR, { recursive: true }); } catch {}
...
function _saveSnapshots() {
  ...
  } catch (e) { log(`snapshot save err: ${e.message}`); }
...
function _loadSnapshots() {
  try {
    if (fs.existsSync(SNAPSHOT_FILE)) {
      ...
      if (loaded > 0) log(`snapshots: merged ${loaded} from disk (total=${_quotaSnapshots.size})`);
    }
  } catch (e) { log(`snapshot load err: ${e.message}`); }
}
...
function _saveInUse(store) {
  ...
  } catch (e) { log(`inUse save err: ${e.message}`); }
  _inUseDirty = false;
}
...
function _loadInUse(store) {
  try {
    if (fs.existsSync(INUSE_FILE)) {
      ...
      if (loaded > 0) log(`inUse: merged ${loaded} marks from disk (total=${store._inUse.size})`);
    }
  } catch (e) { log(`inUse load err: ${e.message}`); }
}
...
function isWeeklyDrought() {
  ...
  if (!_store) { _droughtCache = { value: false, ts: now }; return false; }
  let checked = 0, wDry = 0;
  ...
  if (result) log(`🏜️ Weekly干旱: ${wDry}/${checked}号W耗尽(${Math.round(wDry/checked*100)}%), 周重置${hoursUntilWeeklyReset().toFixed(1)}h后`);
  return result;
}
...
function isClaudeAvailable(health) {
  ...
  if (plan === 'free' && (health.daily || 0) === 0 && (health.weekly || 0) === 0) return false;
  // 付费计划: Claude始终可用
...
function isAccountSwitchable(health) {
  ...
  if (health.daily <= AUTO_SWITCH_THRESHOLD && health.weekly > AUTO_SWITCH_THRESHOLD) {
    if (hoursUntilDailyReset() <= WAIT_RESET_HOURS) return true;
  }
  // Daily耗尽但即将重置 + 干旱模式 → 等重置
  if (isWeeklyDrought() && health.daily <= AUTO_SWITCH_THRESHOLD && hoursUntilDailyReset() <= WAIT_RESET_HOURS) return true;
  return false;
}
...
function isWamMode() { return _mode === 'wam'; }

// ── 官方模式: 彻底隔离第三方套层, 回归本源 ──
function cleanupThirdPartyState() {
  ...
  try { _saveSnapshots(); } catch {}
  try { if (_store) _saveInUse(_store); } catch {}
  _burstUntil = 0;
  _consecutiveChanges = 0;
  // 清除旧版本可能设置的代理环境变量污染
  // 注: 仅清除WAM设置的值(127.0.0.1:7890), 不动用户自己设的代理
  if (process.env.HTTP_PROXY === 'http://127.0.0.1:7890') { delete process.env.HTTP_PROXY; cleaned++; log('cleanup: removed HTTP_PROXY'); }
  if (process.env.HTTPS_PROXY === 'http://127.0.0.1:7890') { delete process.env.HTTPS_PROXY; cleaned++; log('cleanup: removed HTTPS_PROXY'); }
  log(`cleanup: ${cleaned} items cleaned, engines stopped, state cleared — 回归本源`);
  return cleaned;
}
...
class AccountStore {
  constructor(globalStoragePath) {
    ...
    this._sharedPath = path.join(globalStoragePath, '..', 'windsurf-login-accounts.json');
    this.accounts = [];
    this.switchCount = 0;
    this.activeIndex = -1;
    this.lastRefresh = 0;
  ...
  load() {
    ...
    try {
      ...
      if (fs.existsSync(archPath)) {
        ...
        if (Array.isArray(arch)) arch.forEach(a => { if (a.email) blacklist.add(a.email.toLowerCase()); });
      }
    } catch {}
    const seen = new Set();
    const merged = [];
    let sources = 0, blocked = 0;
        ...
        if (fs.existsSync(p)) {
          ...
          if (Array.isArray(data)) {
            ...
            for (const a of data) {
              ...
              if (blacklist.has(e)) { blocked++; continue; } // 黑名单: 已归档死号不再入池
              ...
              if (cachedPlan === 'free') { blocked++; continue; }
              ...
              const _rd = (f) => f == null ? -1 : typeof f === 'number' ? f : (f && f.remaining != null ? f.remaining : -1);
              const cachedD = a.usage ? _rd(a.usage.daily) : -1;
              const cachedW = a.usage ? _rd(a.usage.weekly) : -1;
              if (cachedPlanEnd > 0 && Date.now() > cachedPlanEnd && isTrialPlan(cachedPlan) && cachedD === 0 && cachedW === 0) { blocked++; continue; }
              // v7.2: 格式迁移 — 统一 usage.daily/weekly 为 {remaining: N} 对象格式
              if (a.usage && typeof a.usage.daily === 'number') { a.usage.daily = { remaining: a.usage.daily }; a._migrated = true; }
              if (a.usage && typeof a.usage.weekly === 'number') { a.usage.weekly = { remaining: a.usage.weekly }; a._migrated = true; }
              if (e && !seen.has(e)) {
                seen.add(e);
                merged.push(a);
                added++;
    ...
    log(`store: loaded ${this.accounts.length} accounts (${this.pwCount()} with pw) from ${sources} sources${blocked > 0 ? ` (${blocked} blacklisted)` : ''}`);
    // v7.2.1: 格式迁移后立即持久化，避免file watcher反复读旧格式
    if (this.accounts.some(a => a.usage && a._migrated)) {
      this.accounts.forEach(a => { if (a._migrated) delete a._migrated; });
      this.save();
  ...
  _autoBackup() {
    try {
      ...
      // 只保留最近3个备份
      const files = fs.readdirSync(backupDir).filter(f => f.startsWith('accounts_')).sort();
  ...
  get(i) { return this.accounts[i] || null; }
  count() { return this.accounts.length; }
  pwCount() { return this.accounts.filter(a => a.password).length; }
  ...
  add(email, password) {
    ...
    if (this.accounts.some(a => (a.email || '').toLowerCase() === email.toLowerCase())) return false;
    this.accounts.push({
      email, password,
      loginCount: 0,
      addedAt: Date.now(),
  ...
  async _asyncVerifyAccount(email, password) {
    try {
      ...
      if (quota.ok) {
        ...
        if ((plan === 'free' && quota.daily === 0 && quota.weekly === 0) || (isExpired && isTrialPlan(plan) && quota.daily === 0 && quota.weekly === 0)) {
          ...
          this.save();
          vscode.window.showWarningMessage(`WAM: 已拒绝 ${email} — ${plan === 'free' ? 'Free计划无Claude' : '试用已过期'}`);
          refreshAll();
          return;
        }
        // 验证通过 → 清除未验证标记
        ...
        this.save();
        log(`verify-gate: ${email} → 通过 (plan=${plan}, D${quota.daily}W${quota.weekly})`);
      } else {
        log(`verify-gate: ${email} → 探测失败: ${quota.error} (保留, 等待下次purge)`);
        // 登录永久失败 → 立即清除
        if (/INVALID|NOT_FOUND|DISABLED|WRONG/.test(quota.error || '')) {
          log(`verify-gate: ${email} → 登录死号, 归档移除`);
          this._archiveRemoved([this.accounts[idx]], `verify_gate_dead: ${quota.error}`);
          this.accounts.splice(idx, 1);
          if (this.activeIndex === idx) this.activeIndex = -1;
          else if (this.activeIndex > idx) this.activeIndex--;
          this.save();
          vscode.window.showWarningMessage(`WAM: 已拒绝 ${email} — 登录失败(${quota.error})`);
          refreshAll();
        }
      }
    } catch (e) {
  ...
  _archiveRemoved(accounts, reason) {
    try {
      const archivePath = path.join(path.dirname(this._path), '_wam_purged.json');
      let existing = [];
  ...
  _parseLine(line) {
    ...
    const _ok = s => { const a = s.indexOf('@'); return a > 0 && s.indexOf('.', a) > a && !/[\s:,|]/.test(s); };
    ...
    if (tabs.length >= 2) {
      ...
      if (eI >= 0) return { email: tabs[eI].trim(), password: tabs[eI === 0 ? 1 : 0].trim() };
    }
  ...
  getHealth(acc) {
    ...
    const checked = !!(acc.usage && (acc.usage.daily || acc.usage.lastChecked || acc.loginCount > 0)) || !!snap;
    // v7.2: 兼容两种格式 — usage.daily 可能是数字(82)或对象({remaining:82})
    const _readQuota = (field) => {
      if (field == null) return undefined;
      if (typeof field === 'number') return field;  // 数字格式: 82
      if (typeof field === 'object' && field.remaining != null) return field.remaining; // 对象格式: {remaining:82}
      return undefined;
    };
    ...
    const dailyResetIn = resetTime > now ? Math.round((resetTime - now) / 1000) : 0;
    const weeklyResetIn = weeklyReset > now ? Math.round((weeklyReset - now) / 1000) : 0;
    return {
      checked, daily: dr, weekly: wr, plan,
      daysLeft: Math.round(daysLeft * 10) / 10,
      age, staleMin,
      planEnd, resetTime, weeklyReset, lastChecked,
      dailyResetIn, weeklyResetIn,
      hasSnap: !!snap,
    };
  }

  getPoolStats() {
    let totalD = 0, totalW = 0, available = 0, exhausted = 0, waiting = 0, pwCount = 0, unchecked = 0;
    const drought = isWeeklyDrought();
    for (const a of this.accounts) {
      pwCount++;
      if (!a.password) continue;
      const h = this.getHealth(a);
      if (!h.checked) { unchecked++; continue; }
      totalD += h.daily;
      totalW += h.weekly;
      if (!isClaudeAvailable(h)) exhausted++;
      else if (isAccountSwitchable(h)) available++;
      else if (drought ? (h.daily <= AUTO_SWITCH_THRESHOLD) : (h.weekly <= 2)) exhausted++;
      else waiting++;
    }
    // v7.2: 优先用活跃账号的API重置时间, 仅在无API数据时用计算值兜底
    const now = Date.now();
    const activeAcc = this.activeIndex >= 0 ? this.accounts[this.activeIndex] : null;
    ...
    const hrsD = apiDailyMs > now ? (apiDailyMs - now) / 3600000 : hoursUntilDailyReset();
    const hrsW = apiWeeklyMs > now ? (apiWeeklyMs - now) / 3600000 : hoursUntilWeeklyReset();
    return {
      totalD: Math.round(totalD),
      totalW: Math.round(totalW),
      count: this.accounts.length,
      pwCount, available, exhausted, waiting,
      unchecked, drought,
  ...
  cleanInUse() {
    ...
    if (cleaned) { _inUseDirty = true; _schedulePersist(); }
  }
  ...
  getBestIndex(excludeIndex = -1, skipInUse = true) {
    ...
    for (let i = 0; i < this.accounts.length; i++) {
      ...
      if (drought) {
        ...
        if (h.daily <= 0 && h.weekly <= 0 && hrsToDaily > 4 && hrsToWeekly > 4) continue;
...
function _writeInstanceClaim(email) {
  try {
    fs.mkdirSync(WAM_DIR, { recursive: true });
    let claims = {};
    try { claims = JSON.parse(fs.readFileSync(INSTANCE_LOCK_FILE, 'utf8')); } catch {}
    claims[_instanceId] = { email: (email || '').toLowerCase(), ts: Date.now(), pid: process.pid };
    fs.writeFileSync(INSTANCE_LOCK_FILE, JSON.stringify(claims, null, 2), 'utf8');
  } catch (e) { log(`instance-claim write error: ${e.message}`); }
}
  ...
  try {
    ...
    for (const [instId, claim] of Object.entries(claims)) {
      if (now - claim.ts > INSTANCE_DEAD_MS) {
        ...
        changed = true;
        log(`instance-clean: removed dead ${instId} (${Math.round((now - claim.ts) / 1000)}s stale)`);
      }
    }
    if (changed) fs.writeFileSync(INSTANCE_LOCK_FILE, JSON.stringify(claims, null, 2), 'utf8');
  } catch {}
}
...
function _httpsPost(url, body, opts = {}) {
  return new Promise((resolve, reject) => {
    ...
    const reqOpts = {
      ...
      servername: opts.servername !== undefined ? opts.servername : parsed.hostname,
    };
...
function _httpsViaProxy(proxyHost, proxyPort, targetUrl, body, timeout = 12000) {
  return new Promise((resolve, reject) => {
    const parsed = new URL(targetUrl);
    const timer = setTimeout(() => { reject(new Error('proxy_timeout')); }, timeout);
    ...
    connReq.on('connect', (res, socket) => {
      ...
      const req = https.request({
        ...
        resp.on('end', () => {
          clearTimeout(timer);
          try { resolve(JSON.parse(data)); }
          catch { resolve({ _raw: data, _status: resp.statusCode }); }
        });
      });
      req.on('error', (e) => { clearTimeout(timer); reject(e); });
      req.on('timeout', () => { clearTimeout(timer); req.destroy(); reject(new Error('req_timeout')); });
      req.write(body);
      req.end();
    });
    connReq.on('error', (e) => { clearTimeout(timer); reject(e); });
    connReq.on('timeout', () => { clearTimeout(timer); connReq.destroy(); reject(new Error('proxy_conn_timeout')); });
    connReq.end();
  });
}
...
function _detectProxy() {
  if (_proxyPortCache !== null && (Date.now() - _proxyPortCacheTs) < PROXY_CACHE_TTL) return Promise.resolve(_proxyPortCache);
  ...
  return new Promise((resolve) => {
    ...
    for (const port of PROXY_PORTS) {
      ...
      s.connect(port, PROXY_HOST, () => {
        s.destroy();
        if (!found) { found = true; _proxyPortCache = port; _proxyPortCacheTs = Date.now(); resolve(port); }
      });
      s.on('error', () => { s.destroy(); if (--pending === 0 && !found) { _proxyPortCache = 0; _proxyPortCacheTs = Date.now(); resolve(0); } });
      s.on('timeout', () => { s.destroy(); if (--pending === 0 && !found) { _proxyPortCache = 0; _proxyPortCacheTs = Date.now(); resolve(0); } });
    }
  });
}
...
function _httpsPostRawViaProxy(proxyHost, proxyPort, targetUrl, body, timeout = 12000) {
  return new Promise((resolve, reject) => {
    const parsed = new URL(targetUrl);
    const timer = setTimeout(() => { reject(new Error('proxy_raw_timeout')); }, timeout);
    const connReq = http.request({
      host: proxyHost, port: proxyPort,
      method: 'CONNECT',
      path: `${parsed.hostname}:443`,
    ...
    connReq.on('connect', (res, socket) => {
      if (res.statusCode !== 200) {
        ...
        reject(new Error(`proxy_connect_${res.statusCode}`)); return;
      }
      const req = https.request({
        socket, hostname: parsed.hostname,
        path: parsed.pathname + parsed.search,
        method: 'POST',
        headers: {
          'Content-Type': 'application/proto',
        ...
        resp.on('end', () => { clearTimeout(timer); resolve({ buf: Buffer.concat(chunks), status: resp.statusCode }); });
      });
      req.on('error', (e) => { clearTimeout(timer); reject(e); });
      req.on('timeout', () => { clearTimeout(timer); req.destroy(); reject(new Error('req_timeout')); });
...
function encodeProtoString(str) {
  ...
  const lenBytes = [];
  let l = b.length;
  while (l > 127) { lenBytes.push((l & 0x7f) | 0x80); l >>= 7; }
  lenBytes.push(l);
  ...
  while (pos < buf.length) {
    ...
    switch (wireType) {
      case 0: { // varint
        ...
        if (nextPos === pos) { pos = buf.length; break; }
        pos = nextPos;
        varints[fieldNum] = val;
        break;
      }
      case 1: // 64-bit fixed
        if (pos + 8 > buf.length) { pos = buf.length; break; }
        pos += 8; break;
      case 2: { // length-delimited (string/bytes/nested)
        const [len, nextPos] = readVarint(buf, pos);
        if (nextPos === pos || nextPos + len > buf.length) { pos = buf.length; break; }
        if (!messages[fieldNum]) messages[fieldNum] = buf.slice(nextPos, nextPos + len);
        pos = nextPos + len;
        break;
      }
      case 5: // 32-bit fixed
        if (pos + 4 > buf.length) { pos = buf.length; break; }
        pos += 4; break;
      default: pos = buf.length; // unknown wire type → bail
    }
  }
...
function _extractQuotaFields(v, msgs) {
  const dailyR = v[14], weeklyR = v[15], used = v[6], total = v[8], dReset = v[17], wReset = v[18];
  let valid = 0;
  ...
  let planName = null;
  if (msgs) {
    const knownPlans = /^(free|pro_trial|pro|trial|enterprise|team|individual)$/i;
    for (const fn of Object.keys(msgs)) {
      try {
        const str = msgs[fn].toString('utf8').trim();
        if (knownPlans.test(str)) { planName = str; break; }
      } catch {}
    }
  }

  const dailyVal = (dailyR !== undefined && dailyR >= 0 && dailyR <= 100) ? dailyR : 0;
  ...
  let planStartUnix = 0, planEndUnix = 0;
  if (msgs) {
    try {
      if (msgs[2]) {
    ...
    if (planStartUnix && planEndUnix) {
      ...
      if (planStartUnix >= planEndUnix || durDays > 365) {
        log(`planDate suspect: start=${new Date(planStartUnix*1000).toISOString().slice(0,10)} end=${new Date(planEndUnix*1000).toISOString().slice(0,10)} dur=${durDays.toFixed(1)}d → discard`);
        planStartUnix = 0;
        planEndUnix = 0;
      }
    }
...
function parsePlanStatus(buf) {
  ...
  if (pb.length >= 5) {
    ...
    if ((flags === 0x00 || flags === 0x02) && msgLen > 0 && msgLen + 5 <= pb.length) {
  ...
  if (top.messages[1] && top.messages[1].length > 10) {
    ...
    if (result) {
      log(`proto L3: D${result.daily} W${result.weekly}${result.planName ? ' plan=' + result.planName : ''}${result.planEndUnix ? ' end=' + new Date(result.planEndUnix * 1000).toISOString().slice(0,10) : ''} env=${stripped} ${pb.length}B`);
      return result;
    }
    // 层4: 再深一层
    if (inner.messages[1] && inner.messages[1].length > 10) {
      ...
      if (result) {
        log(`proto L4: D${result.daily} W${result.weekly}${result.planName ? ' plan=' + result.planName : ''}${result.planEndUnix ? ' end=' + new Date(result.planEndUnix * 1000).toISOString().slice(0,10) : ''} env=${stripped} ${pb.length}B`);
        return result;
      }
    }
  }
  ...
  if (result) {
    log(`proto L2: D${result.daily} W${result.weekly}${result.planEndUnix ? ' end=' + new Date(result.planEndUnix * 1000).toISOString().slice(0,10) : ''} env=${stripped} ${pb.length}B b0=0x${buf[0].toString(16)}`);
    return result;
  }

  log(`parsePlanStatus: no quota in ${pb.length}B env=${stripped} b0=0x${buf[0].toString(16)} top=${Object.keys(top.varints).join(',')} hex=${pb.slice(0, 48).toString('hex')}`);
  return null;
}
...
async function firebaseLogin(email, password) {
  ...
  for (const key of FIREBASE_KEYS) {
    ...
    const racePromises = channels.map(ch =>
      _firebaseVia(ch, email, password, key)
        .then(result => {
          if (result && result.idToken) return { ok: true, idToken: result.idToken, channel: `${ch}-${keySuffix}` };
        ...
        .catch(e => { errors[`${ch}-${keySuffix}`] = e.message; throw e; })
    );

    try {
      const result = await Promise.any(racePromises);
      if (result.ok) return result;
      if (result.permanent) return { ok: false, error: result.error, channel: 'permanent' };
    } catch (aggErr) {
      const permanentErr = Object.values(errors).find(e => /INVALID|NOT_FOUND|DISABLED|WRONG/.test(e));
      if (permanentErr) return { ok: false, error: permanentErr, channel: 'permanent' };
    }
  }
  ...
  for (const key of FIREBASE_KEYS) {
    ...
    for (const ch of channels) {
      try {
        const result = await _firebaseVia(ch, email, password, key);
        if (result && result.idToken) return { ok: true, idToken: result.idToken, channel: `retry-${ch}-${keySuffix}` };
      } catch {}
    }
  }
...
async function getCachedToken(email, password) {
  ...
  if (cached && cached.expiresAt > Date.now()) return { ok: true, idToken: cached.idToken };
  const loginResult = await firebaseLogin(email, password);
  if (!loginResult.ok) return loginResult;
  _tokenCache.set(key, { idToken: loginResult.idToken, expiresAt: Date.now() + TOKEN_CACHE_TTL });
  return loginResult;
}
...
async function _resolveRelayIP() {
  if (_relayIPCache.ip && Date.now() - _relayIPCache.ts < RELAY_IP_TTL) return _relayIPCache.ip;
  ...
  try {
    ...
    if (port) {
      const dohResult = await new Promise((resolve, reject) => {
        ...
        const connReq = http.request({
          host: PROXY_HOST, port, method: 'CONNECT', path: 'dns.google:443', timeout: 5000,
        });
        connReq.on('connect', (res, socket) => {
          if (res.statusCode !== 200) { clearTimeout(timer); socket.destroy(); reject(new Error('doh_proxy')); return; }
          const req = https.request({
            socket, hostname: 'dns.google',
            path: `/resolve?name=${RELAY_HOST}&type=A`, method: 'GET',
            headers: { Host: 'dns.google' },
            servername: 'dns.google', rejectUnauthorized: false, timeout: 6000,
          }, (resp) => {
            let d = '';
            resp.on('data', c => d += c);
            resp.on('end', () => {
              clearTimeout(timer);
              try { const j = JSON.parse(d); resolve(j.Answer ? j.Answer[0].data : null); }
              catch { resolve(null); }
            });
          });
          req.on('error', e => { clearTimeout(timer); reject(e); });
      ...
      if (dohResult && /^\d+\.\d+\.\d+\.\d+$/.test(dohResult)) {
        _relayIPCache = { ip: dohResult, ts: Date.now() };
        log(`relay IP resolved via DoH: ${dohResult}`);
        return dohResult;
      }
    }
  } catch (e) { log(`DoH resolve err: ${e.message}`); }
...
async function fetchAccountQuota(email, password) {
  ...
  if (cd && now < cd.nextAllowedTs) {
    return { ok: false, error: 'rate_limited', retryAfter: cd.nextAllowedTs - now };
  }
  ...
  // v7.2: 通道优先级 — 直连Relay真实IP > 代理→Relay
  // 诊断实证: register.codeium.com已NXDOMAIN, 代理→relay被429限流
  // 直连relay真实IP (绕过Clash) 是唯一可靠通道
  const channels = [
    // 通道1: 直连Relay真实IP (DoH解析绕过fake-ip)
    async () => {
    ...
    // 通道2: 代理→Relay (fallback)
    async () => {
      const port = await _detectProxy();
      if (!port) throw new Error('no_proxy');
      return _httpsPostRawViaProxy(PROXY_HOST, port, planUrl, proto, 10000);
    },
  ];
  ...
  for (let i = 0; i < channels.length; i++) {
    const chName = `ch${i + 1}`;
    try {
      const resp = await channels[i]();
      if (resp.status === 429) {
        _quotaFetchCooldown.set(key, { nextAllowedTs: Date.now() + QUOTA_429_BACKOFF });
        log(`${chName}: 429 → backoff ${QUOTA_429_BACKOFF / 1000}s`);
        // 429只对当前通道退避，继续尝试下一通道
        continue;
      }
      if (resp.status === 200 && resp.buf && resp.buf.length > 50) {
        ...
        if (q) {
          ...
          log(`${chName}: OK D${q.daily} W${q.weekly}${q.planName ? ' ' + q.planName : ''}`);
...
function _updateAccountUsage(email, quota) {
  if (!_store) return;
  const acc = _store.accounts.find(a => a.email.toLowerCase() === email.toLowerCase());
  if (!acc) return;
  ...
  const apiWeeklyReset = quota.weeklyResetUnix ? quota.weeklyResetUnix * 1000 : 0;
  const calcDailyReset = getNextDailyResetMs();
  const calcWeeklyReset = getNextWeeklyResetMs();

  // v7.2: 始终信任API重置时间, 仅在API无值时用计算值兜底
  const effectiveWeeklyReset = apiWeeklyReset || calcWeeklyReset || prev.weeklyReset || 0;
  ...
  acc.usage = {
    ...
    planEnd: (() => {
      ...
      if (pe > 0 && pe < Date.now() && (quota.daily > 0 || quota.weekly > 0)) return 0; // 宽限期: 清除过期planEnd
  ...
  for (const cmd of commands) {
    for (let attempt = 1; attempt <= 2; attempt++) {
      try {
        ...
        log(`inject: ${cmd} attempt ${attempt}`);
        const result = await Promise.race([
          vscode.commands.executeCommand(cmd, idToken),
          new Promise((_, rej) => setTimeout(() => rej(new Error('注入超时30s')), 30000)),
        ]);
        if (result && result.error) {
          log(`inject: ${cmd} returned error: ${JSON.stringify(result.error)}`);
          continue;
        }
        // provideAuthTokenToAuthProvider returns {session}
        ...
        // loginWithAuthToken may return session directly
        ...
        log(`inject: ${cmd} unexpected result: ${JSON.stringify(result).substring(0, 200)}`);
      } catch (e) {
        log(`inject: ${cmd} attempt ${attempt} threw: ${e.message}`);
      }
    }
  }
  return { ok: false, error: 'all inject methods failed' };
}
  ...
  for (const key of FIREBASE_KEYS) {
    ...
    try {
      ...
      if (port) {
        ...
        const result = await _httpsViaProxy(PROXY_HOST, port, url2, payload, 10000);
        if (result?.users?.[0]) return result.users[0];
      }
    } catch {}
  }
...
async function verifyAndPurgeExpired(store, opts = {}) {
  if (_purgeRunning) { log('purge: already running'); return { purged: 0 }; }
  _purgeRunning = true;
  ...
  if (!silent) broadcastMessage({ type: 'toast', text: `正在验证 ${pwAccounts.length} 个账号...` });
    ...
    if (!toRemoveIndices.includes(i)) {
      try {
        ...
        if (quota.ok) {
          ...
          const daysExpired = isExpired ? (Date.now() - freshPlanEnd) / 86400000 : 0;

          // Case 1: 实时plan是free → 无Claude($$$)权限 → 清理
          if (freshPlan === 'free') {
            toRemoveIndices.push(i);
            reasons[i] = `probe_free: plan=free, D${quota.daily}W${quota.weekly}仅限免费模型, Claude不可用`;
            log(`purge: ${acc.email} → FREE (实时探测: plan=${freshPlan}, D${quota.daily}W${quota.weekly})`);
          }
          // Case 2: 试用过期 — 仅当D=0且W=0时才清理(planEnd过期但有配额仍可用)
          else if (isExpired && isTrialPlan(freshPlan) && quota.daily === 0 && quota.weekly === 0) {
            toRemoveIndices.push(i);
            reasons[i] = `expired_trial_no_quota: plan=${freshPlan}过期${daysExpired.toFixed(1)}天且D0/W0, Claude不可用`;
            log(`purge: ${acc.email} → EXPIRED+D0W0 ${daysExpired.toFixed(1)}d (plan=${freshPlan}, end=${new Date(freshPlanEnd).toISOString().slice(0,10)})`);
          }
          // Case 2b: 试用过期但仍有配额 → 保留(Windsurf宽限期)
          else if (isExpired && isTrialPlan(freshPlan)) {
            log(`purge: ${acc.email} → EXPIRED_BUT_USABLE (plan=${freshPlan} expired ${daysExpired.toFixed(1)}d, but D${quota.daily}W${quota.weekly}>0, 保留)`);
          }
          // Case 3: Claude可用
          else {
            log(`purge: ${acc.email} → CLAUDE✓ (plan=${freshPlan}${isExpired ? ' expired!' : ''} D${quota.daily}W${quota.weekly})`);
          }
        } else {
          log(`purge: ${acc.email} → probe failed: ${quota.error} (保留, 不冤枉)`);
        }
      } catch (e) {
  ...
  for (let i = 0; i < store.accounts.length; i++) {
    ...
    if (h.planEnd > 0 && h.daysLeft <= 0 && isTrialPlan(h.plan) && (h.daily || 0) === 0 && (h.weekly || 0) === 0) {
      toRemoveIndices.push(i);
      reasons[i] = `cached_expired_no_quota: plan=${h.plan}过期${Math.abs(h.daysLeft).toFixed(1)}天且D0/W0, Claude不可用`;
      log(`purge: ${acc.email} → EXPIRED+D0W0 (缓存兜底: plan=${h.plan}, daysLeft=${h.daysLeft})`);
    }
  }
  ...
  if (toRemoveIndices.length > 0) {
    ...
    for (const idx of sorted) {
      ...
      archived.push({ ...acc, _purgeReason: reasons[idx], _purgedAt: Date.now() });
  ...
  const probeFree = rv.filter(r => r.startsWith('probe_free') || r.startsWith('cached_free')).length;
  const expiredTrial = rv.filter(r => r.startsWith('expired_trial') || r.startsWith('cached_expired')).length;
...
function _archivePurged(store, archived) {
  try {
    const archivePath = path.join(path.dirname(store._path), '_wam_purged.json');
    let existing = [];
    try { existing = JSON.parse(fs.readFileSync(archivePath, 'utf8')); } catch {}
    if (!Array.isArray(existing)) existing = [];
  ...
  } catch (e) { log(`purge archive error: ${e.message}`); }
}
...
async function switchToAccount(email, password) {
  ...
  if (!loginResult.ok) {
    ...
    // 永久性认证错误 → 归档此账号 (不直接删除，防止网络波动误杀)
    if (/INVALID|NOT_FOUND|DISABLED|WRONG/.test(err)) {
      const idx = _store?.accounts.findIndex(a => a.email.toLowerCase() === email.toLowerCase());
      if (idx >= 0) {
        const deadAcc = _store.accounts[idx];
        log(`switch: archiving dead account [${idx}] ${email} (${err})`);
        _archivePurged(_store, [{ ...deadAcc, _purgeReason: `switch_dead: ${err}`, _purgedAt: Date.now() }]);
        _store.remove(idx);
        vscode.window.showWarningMessage(`WAM: 已归档无效账号 ${email} (${err})，可从_wam_purged.json恢复`);
      }
    }
    return { ok: false, error: `登录失败: ${err}`, ms: Date.now() - t0, permanent: /INVALID|NOT_FOUND|DISABLED|WRONG/.test(err) };
  }
  log(`switch: login OK via ${loginResult.channel} (${loginResult.idToken.length}ch) [${Date.now()-t0}ms]`);
  const injectResult = await injectAuth(loginResult.idToken);
  const ms = Date.now() - t0;
  if (!injectResult.ok) {
    log(`switch FAIL inject: ${JSON.stringify(injectResult.error)}`);
    return { ok: false, error: `注入失败: ${JSON.stringify(injectResult.error)}`, ms };
  }
  log(`switch OK: ${injectResult.account} apiKey=${injectResult.apiKey.substring(0, 20)}... ${ms}ms`);
  _lastSwitchTime = Date.now(); // 标记切号时间
  _writeInstanceClaim(email); // 跨实例声明占用
  try {
    fs.mkdirSync(WAM_DIR, { recursive: true });
    fs.writeFileSync(RESULT_FILE, JSON.stringify({
      ok: true, ts: Date.now(), email,
      account: injectResult.account,
      apiKey: injectResult.apiKey.substring(0, 25) + '...',
      sessionId: injectResult.sessionId,
    }));
  } catch {}
  return { ok: true, account: injectResult.account, apiKey: injectResult.apiKey, ms };
}
...
async function monitorActiveQuota() {
  ...
  try {
    ...
    if (activeI < 0) { _monitorActive = false; return; }
    const acc = _store.get(activeI);
    if (!acc || !acc.password) { _monitorActive = false; return; }
    ...
    const now = Date.now();
    _quotaSnapshots.set(emailKey, { daily: result.daily, weekly: result.weekly, ts: now });
    _snapshotDirty = true;
    _schedulePersist();
    ...
    if (prev) {
      ...
      const hasFluctuation = dDelta > CHANGE_THRESHOLD || wDelta > CHANGE_THRESHOLD;
      ...
      if (hasFluctuation) {
        ...
        log(`📊 D${prev.daily}→${result.daily}(Δ${dDelta.toFixed(1)}) W${prev.weekly}→${result.weekly}(Δ${wDelta.toFixed(1)}) ${acc.email.substring(0, 25)} [×${_consecutiveChanges}]`);
        broadcastMessage({ type: 'quotaChange', email: acc.email,
          prevD: prev.daily, curD: result.daily, prevW: prev.weekly, curW: result.weekly });

        // ── 消息锚定核心: 波动=有人发消息→立即切到新账号, 确保下条消息用新号 ──
        if (autoRotate && !_switching) {
          let bestI = _predictiveCandidate >= 0 ? _predictiveCandidate : -1;
          if (bestI >= 0) {
            const candAcc = _store.get(bestI);
            if (!candAcc || !candAcc.password || _isClaimedByOther(candAcc.email)) bestI = -1;
          }
          if (bestI < 0) bestI = _store.getBestIndex(activeI, true);

          if (bestI >= 0) {
            const bestAcc = _store.get(bestI);
            log(`⚡ 消息锚定切号: D${result.daily}%·W${result.weekly}% → ${bestAcc.email.substring(0, 20)}${_predictiveCandidate >= 0 ? ' [预判]' : ''}`);
            _switching = true;
            try {
              const switchResult = await switchToAccount(bestAcc.email, bestAcc.password);
              if (switchResult.ok) {
                ...
                if (_predictiveCandidate >= 0) {
                  log(`🔮 预选下一个: → ${_store.get(_predictiveCandidate).email.substring(0, 20)}`);
                }
                ...
                vscode.window.showWarningMessage(`WAM: 切换失败 — ${switchResult.error}`);
      ...
      const effQuota = drought ? result.daily : Math.min(result.daily, result.weekly);
      if (effQuota < PREDICTIVE_THRESHOLD && _predictiveCandidate < 0 && autoRotate) {
        _predictiveCandidate = _store.getBestIndex(activeI, true);
        if (_predictiveCandidate >= 0) {
          log(`🔮 预判: 额度${effQuota.toFixed(0)}%<${PREDICTIVE_THRESHOLD}%, 预选→${_store.get(_predictiveCandidate).email.substring(0, 20)}`);
        }
      }
      ...
      if (isExhausted && autoRotate && !_switching) {
        const hrsToReset = hoursUntilDailyReset();

        if (result.daily < AUTO_SWITCH_THRESHOLD && hrsToReset <= WAIT_RESET_HOURS) {
          log(`⏳ Daily耗尽(${result.daily}%) 但${hrsToReset.toFixed(1)}h后重置 → 等待`);
        } else if (!drought && result.daily >= AUTO_SWITCH_THRESHOLD && result.weekly < AUTO_SWITCH_THRESHOLD && hoursUntilWeeklyReset() <= WAIT_RESET_HOURS) {
          log(`⏳ Weekly耗尽(${result.weekly}%) 但${hoursUntilWeeklyReset().toFixed(1)}h后重置 → 等待`);
        } else {
          const reason = drought
            ? `Daily耗尽(${result.daily}%)`
            : (result.weekly < AUTO_SWITCH_THRESHOLD ? `Weekly耗尽(${result.weekly}%)` : `Daily耗尽(${result.daily}%)`);
          let bestI = _predictiveCandidate >= 0 ? _predictiveCandidate : _store.getBestIndex(activeI, true);
          if (bestI >= 0) {
            const bestAcc = _store.get(bestI);
            ...
            try {
              ...
              if (sr.ok) {
                ...
                _burstUntil = Date.now() + BURST_DURATION;
                setTimeout(() => monitorActiveQuota(), 1500);
                vscode.window.showInformationMessage(`WAM: ${reason} → 切换到 ${sr.account}`);
                refreshAll();
              } else {
...
async function scanBackgroundQuota() {
  ...
  try {
    ...
    if (pwAccounts.length === 0) { _scanRunning = false; return; }
    ...
    for (const acc of batch) {
      if (_switching) break;
      // 跳过当前活跃账号(已由快速监测覆盖)
      const activeAcc = _store.activeIndex >= 0 ? _store.get(_store.activeIndex) : null;
      if (activeAcc && acc.email.toLowerCase() === activeAcc.email.toLowerCase()) continue;
      ...
      try {
        const result = await fetchAccountQuota(acc.email, acc.password);
        if (result.ok) {
          _quotaSnapshots.set(emailKey, { daily: result.daily, weekly: result.weekly, ts: Date.now() });
          _snapshotDirty = true;
          ...
          if (baseD >= 0 && baseW >= 0) {
            ...
            if (Math.abs(dDelta) > CHANGE_THRESHOLD || Math.abs(wDelta) > CHANGE_THRESHOLD) {
              _store.markInUse(acc.email);
              changed++;
              const src = prev ? '' : '(baseline)';
              log(`scan: ${acc.email.substring(0, 25)} CHANGED${src} D${baseD}→${result.daily}(Δ${dDelta.toFixed(1)}) W${baseW}→${result.weekly}(Δ${wDelta.toFixed(1)}) → 标记使用中`);
            }
          }
          scanned++;
        }
    ...
    for (const [email, info] of _store._inUse) {
      if (now - info.lastChange > INUSE_COOLDOWN_MS) {
        ...
        expiredCount++;
        log(`inUse expired: ${email.substring(0, 25)} (${Math.round((now - info.lastChange) / 1000)}s idle)`);
      }
    }
    if (expiredCount > 0) { _inUseDirty = true; _schedulePersist(); }

    if (scanned > 0 || changed > 0) {
      log(`scan: batch[${_scanOffset - SCAN_BATCH_SIZE}+${batch.length}] ${scanned}ok ${changed}changed inUse=${_store._inUse.size}`);
    }
    _store.save();
    refreshAll();
  } catch (e) {
...
async function scanMissingExpiry() {
  if (!_store || _expiryScanning) { log('scanExpiry: already running'); return { scanned: 0, fetched: 0, failed: 0 }; }
  ...
  if (targets.length === 0) { _expiryScanning = false; return { scanned: 0, fetched: 0, failed: 0 }; }
  broadcastMessage({ type: 'toast', text: `正在刷新 ${targets.length} 个账号有效期...` });
  ...
  for (const idx of targets) {
    ...
    try {
      ...
      if (result.ok) {
        ...
        log(`scanExpiry: ${acc.email.substring(0, 25)} OK D${result.daily} W${result.weekly} planEnd=${pe2 > 0 ? new Date(pe2).toISOString().slice(0, 10) : 'NONE'}`);
      } else {
        failed++;
    ...
    if ((fetched + failed) % 5 === 0) {
      broadcastMessage({ type: 'toast', text: `有效期刷新: ${fetched + failed}/${targets.length} (${fetched}成功 ${failed}失败)` });
      _store.save();
      refreshAll();
    }
    await new Promise(r => setTimeout(r, 400));
...
function updateStatusBar() {
  ...
  const modeLabel = _mode === 'official' ? '[官方]' : '';
  ...
  if (activeAcc) {
    ...
    const monTag = _mode === 'official' ? '$(key)' : (_monitorActive ? '$(sync~spin)' : '$(zap)');
    _statusBarItem.text = `${monTag}${modeLabel}${droughtTag} D${liveD}%·W${liveW}% ${s.available}/${s.pwCount}号${inUseTag}${waitTag}`;
    _statusBarItem.tooltip = `WAM v6.0 [${_mode === 'official' ? '官方模式' : 'WAM切号'}]${s.drought ? ' [🏜️Weekly干旱模式·只看D]' : ''}\n` +
      `活跃: ${activeAcc.email}\n${h.plan}\n` +
      `号池: ${s.available}可用 · ${s.exhausted}耗尽 · ${s.waiting}等重置\n` +
      (s.drought ? `⚠️ Weekly全面耗尽 — 自动切号仅看Daily，避免无效轮转\n` : '') +
    ...
    const monTag2 = _mode === 'official' ? '$(key)' : '$(zap)';
    _statusBarItem.text = `${monTag2}${modeLabel} ${s.pwCount}号`;
    _statusBarItem.tooltip = `WAM v6.0 [${_mode === 'official' ? '官方模式' : 'WAM切号'}] · 未选择活跃账号\n日重置: ${s.hrsToDaily.toFixed(1)}h后 · 周重置: ${s.hrsToWeekly.toFixed(1)}h后`;
  }
}
...
async function handleWebviewMessage(msg) {
  switch (msg.type) {
    case 'switch': {
      ...
      if (_mode === 'official') { saveMode('wam'); log('switch: auto→wam mode'); }
      const acc = _store.get(msg.index);
      if (!acc || !acc.password) return;
      if (_switching) { vscode.window.showWarningMessage('WAM: 正在切换中...'); return; }
      _switching = true;
    ...
    case 'remove': {
      const acc = _store.get(msg.index);
      if (!acc) return;
      const pick = await vscode.window.showWarningMessage(`删除 ${acc.email}?`, { modal: true }, '确认删除');
      if (pick === '确认删除') { _store.remove(msg.index); refreshAll(); }
      break;
    }
    case 'removeBatch': {
      if (!msg.indices || !msg.indices.length) return;
      const pick = await vscode.window.showWarningMessage(`批量删除 ${msg.indices.length} 个账号?`, { modal: true }, '确认删除');
      if (pick === '确认删除') {
    ...
    case 'addBatch': {
      ...
      else if (r.duplicate > 0) vscode.window.showWarningMessage(`WAM: ${r.duplicate}个账号已存在，无新增`);
      else vscode.window.showWarningMessage(`WAM: 无法识别格式，请检查输入 (${r.total}行)`);
      refreshAll();
      break;
    }
    case 'refresh': {
    ...
    case 'autoRotate': {
      if (!isWamMode()) { saveMode('wam'); log('autoRotate: auto→wam mode'); }
    ...
    case 'copyAccount': {
      ...
      if (acc2) {
        const text = acc2.password ? `${acc2.email}:${acc2.password}` : acc2.email;
    ...
    case 'setMode': {
      ...
      saveMode(newMode);
      if (newMode === 'wam' && _store.activeIndex < 0 && _store.pwCount() > 0) {
        ...
        if (bestI >= 0) {
          ...
          try {
            ...
            if (result.ok) {
              ...
              _store.switchCount++;
              _store.save();
              vscode.window.showInformationMessage(`WAM: WAM模式启动，自动登录 ${result.account}`);
            }
          } finally { _switching = false; }
        }
      }
      if (newMode === 'official') {
        const cleaned = cleanupThirdPartyState();
        vscode.window.showInformationMessage(`WAM: 官方模式 — ${cleaned}项清理，第三方套层已隔离，请用Windsurf原生登录/登出`);
      }
      refreshAll();
      break;
    }
...
function broadcastMessage(msg) {
  if (_sidebarProvider && _sidebarProvider._view) _sidebarProvider._view.webview.postMessage(msg);
...
function buildHtml(store) {
  ...
  for (const i of allIndices) {
    ...
    const domainBadge = domain.endsWith('.shop') ? 'shop' : domain.includes('yahoo') ? 'yh' : 'o';
    const localPart = a.email.replace(/@.*/, '');
    const emailShort = localPart.substring(0, 12) + (localPart.length > 12 ? '..' : '');
    // getHealth() 已融合快照, 是唯一数据源
    const isUnchecked = !h.checked;
    const dPct = isUnchecked ? 0 : Math.max(0, Math.min(100, Math.round(h.daily)));
    const wPct = isUnchecked ? 0 : Math.max(0, Math.min(100, Math.round(h.weekly)));
    const dColor = isUnchecked ? '#555' : (dPct <= 5 ? '#f44' : dPct <= 30 ? '#ce9178' : '#4ec9b0');
    const wColor = isUnchecked ? '#555' : (wPct <= 5 ? '#f44' : wPct <= 30 ? '#ce9178' : '#4ec9b0');
    const liveTag = h.hasSnap ? '<span class="live-dot" title="实时数据"></span>' : '';
    const cooldownSec = store.getInUseCooldown(a.email);
    const inUseTag = inUse ? `<span class="iu">使用中(${cooldownSec}s)</span>` : '';
    const uncheckedTag = isUnchecked ? '<span class="uc">未验</span>' : '';
    const planTag = h.plan && h.plan !== 'Trial' ? `<span class="plan-tag">${h.plan}</span>` : '';
    // 过期判定: Claude可用性是ground truth, 不再被D/W数字迷惑
    const claudeOk = isClaudeAvailable(h);
    let expiryTag = '';
    if (h.daysLeft > 0) {
      const ec = h.daysLeft <= 2 ? '#f44' : h.daysLeft <= 5 ? '#ce9178' : '#4ec9b0';
      ...
      expiryTag = '<span class="days" style="color:#ce9178" title="宽限期仍可用">已过期</span>';
    } else if (h.planEnd > 0) {
      expiryTag = '<span class="days" style="color:#f44" title="试用已过期·Claude不可用">已过期</span>';
    }
    const claudeTag = !claudeOk && h.checked ? '<span class="days" style="color:#f44;font-weight:700" title="Claude($$$)模型不可用·仅免费模型">⊘Claude</span>' : '';
    const freshTag = h.staleMin >= 0 && h.staleMin <= 3 ? '<span class="fresh">&#8226;</span>' : '';
    ...
    <div class="row${isActive ? ' act' : ''}${inUse ? ' in-use' : ''}${!claudeOk && h.checked ? ' expired-row' : ''}" data-i="${i}" data-email="${a.email.toLowerCase()}">
      ...
      <span class="acts">
        <button class="b sw" onclick="sw(${i})" title="手动切换(无限制)">&#9889;</button>
  ...
  const poolPct = checkedCount > 0 ? Math.round((stats.drought ? stats.totalD : stats.totalW) / checkedCount) : 0;
  const poolColor = poolPct >= 60 ? '#4ec9b0' : poolPct >= 30 ? '#ce9178' : '#f44';

  // 监测状态 + 重置倒计时
  const burstActive = Date.now() < _burstUntil;
  const burstSec = burstActive ? Math.ceil((_burstUntil - Date.now()) / 1000) : 0;
  ...
  // 活跃账号信息
  let activeHtml = '<div class="act-info" style="border-color:#555;color:#666">未选择活跃账号</div>';
  if (activeI >= 0 && accounts[activeI]) {
    ...
    const snapAge = ah.hasSnap ? `${Math.round((Date.now() - ah.lastChecked) / 1000)}秒前` : '无数据';
    // 干旱模式: 有效配额只看Daily (W0是全局问题)
    const effQuota = stats.drought ? liveD : Math.min(liveD, liveW);
    const effColor = effQuota < 5 ? 'var(--red)' : effQuota < 30 ? 'var(--orange)' : 'var(--green)';
    const switchHint = effQuota < 5
      ? (stats.drought ? ' · <b style="color:var(--orange)">干旱·D耗尽即切</b>' : ' · <b style="color:var(--red)">即将切号</b>')
      : (stats.drought ? ' · <span style="color:#d29922;font-size:9px">[干旱·只看D]</span>' : '');
    const activeClaudeOk = isClaudeAvailable(ah);
    const planExpiryTag = ah.daysLeft > 0
      ? ` <span style="color:${ah.daysLeft <= 2 ? 'var(--red)' : ah.daysLeft <= 5 ? 'var(--orange)' : 'var(--green)'}">${ah.daysLeft}天</span>`
      : (ah.daysLeft < 0 ? ' <span style="color:var(--orange)" title="宽限期仍可用">已过期</span>'
        : (ah.planEnd > 0 ? ' <span style="color:var(--red)">已过期</span>' : ''));
    const activeClaudeTag = !activeClaudeOk ? ' <span style="color:var(--red);font-weight:700">⊘Claude不可用</span>' : '';
...
<div class="hd">
  ...
  ${_mode === 'official' ? '<div class="official-banner"><b>&#128274; 官方模式 · 本源隔离</b><br>切号系统/设备指纹/Pool注入/监测引擎 — 全部已停止<br>请使用 Windsurf 原生登录/登出 · 点击 WAM切号 恢复</div>' : ''}
  ...
  _watcher = fs.watch(WAM_DIR, (eventType, filename) => {
    if (filename === 'oneshot_token.json' && eventType === 'rename') {
      setTimeout(async () => {
        ...
        if (!isWamMode()) { log('watcher: skip injection (official mode)'); try { fs.unlinkSync(TOKEN_FILE); } catch {} return; }
        try {
          ...
          fs.writeFileSync(RESULT_FILE, JSON.stringify({
            ok: result.ok, ts: Date.now(),
            email: data.email || '',
            account: result.account || '',
            apiKey: result.ok ? result.apiKey.substring(0, 25) + '...' : '',
            error: result.error || undefined,
            sessionId: result.sessionId || '',
          }));
          log(`watcher: inject ${result.ok ? 'OK' : 'FAIL'}: ${result.account || result.error}`);
          refreshAll();
        } catch (e) {
...
function activate(context) {
  log(`activate v7.2.1-纯切号 — inst=${_instanceId} DoH直连·去除污染·官方登录可完全回退`);
  ...
  loadMode();
  log(`startup mode: ${_mode} | snapshots: ${_quotaSnapshots.size} | inUse: ${_store._inUse.size}`);

  // 检测活跃账号 (从marker文件)
  const markerPaths = [
    path.join(WAM_DIR, '_active_account.txt'),
  ];
  for (const mp of markerPaths) {
    try {
      if (fs.existsSync(mp)) {
        ...
        for (let i = 0; i < _store.accounts.length; i++) {
          if (_store.accounts[i].email === ae) { _store.activeIndex = i; break; }
        }
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('wam.switchAccount', async () => {
      if (_mode === 'official') { saveMode('wam'); log('switchAccount: auto→wam mode'); }
      const items = _store.accounts
        .map((a, idx) => {
          const i = idx;
          const h = _store.getHealth(a);
          const inUse = _store.isInUse(a.email) ? ' [使用中]' : '';
          const liveTag = h.hasSnap ? '●' : '';
          return { label: `${idx+1}. ${a.email}${inUse}`, description: `${liveTag}D${Math.round(h.daily)}%·W${Math.round(h.weekly)}% ${h.plan}`, index: i };
        });
      const pick = await vscode.window.showQuickPick(items, { placeHolder: '手动选择账号 (无任何限制)' });
      if (pick) {
    ...
    vscode.commands.registerCommand('wam.refreshAll', () => {
      ...
      _store.lastRefresh = Date.now();
      refreshAll();
      vscode.window.showInformationMessage(`WAM: 已刷新 ${_store.pwCount()} 个账号`);
    }),
    vscode.commands.registerCommand('wam.addAccount', async () => {
      ...
      if (input) {
        ...
        refreshAll();
        if (r.added > 0) vscode.window.showInformationMessage(`WAM: 添加了 ${r.added} 个账号`);
    ...
    vscode.commands.registerCommand('wam.panicSwitch', async () => {
      if (_mode === 'official') saveMode('wam'); // 紧急切换自动切回WAM模式
      const bestI = _store.getBestIndex(_store.activeIndex, false); // 紧急切换不跳过使用中
      if (bestI < 0) { vscode.window.showErrorMessage('WAM: 无可用账号'); return; }
      const acc = _store.get(bestI);
      _switching = true;
      try {
        const result = await switchToAccount(acc.email, acc.password);
    ...
    vscode.commands.registerCommand('wam.injectToken', async () => {
      if (!fs.existsSync(TOKEN_FILE)) { vscode.window.showWarningMessage('WAM: 无待注入token'); return; }
      const data = JSON.parse(fs.readFileSync(TOKEN_FILE, 'utf8'));
      const result = await injectAuth(data.idToken);
      if (result.ok) {
        try { fs.unlinkSync(TOKEN_FILE); } catch {}
        vscode.window.showInformationMessage(`WAM: 注入成功 — ${result.account}`);
      } else {
        vscode.window.showErrorMessage(`WAM: 注入失败 — ${JSON.stringify(result.error)}`);
      }
      refreshAll();
    }),
    ...
    vscode.commands.registerCommand('wam.officialMode', async () => {
      const choice = await vscode.window.showWarningMessage(
        'WAM: 切换到官方模式？\n将清除设备指纹salt、pool注入、监测引擎等全部第三方套层，回归Windsurf原生登录。',
        { modal: true }, '确认回归本源'
      );
      if (choice !== '确认回归本源') return;
      saveMode('official');
      const cleaned = cleanupThirdPartyState();
      vscode.window.showInformationMessage(`WAM: 官方模式已激活 — ${cleaned}项清理完成，所有第三方套层已隔离，请使用Windsurf原生登录`);
      refreshAll();
    }),
    ...
    vscode.commands.registerCommand('wam.status', () => {
      ...
      const activeAcc = _store.activeIndex >= 0 ? _store.get(_store.activeIndex) : null;
      const inUseEmails = [..._store._inUse.keys()].map(e => e.substring(0, 15)).join(', ');
      let msg = `WAM v7.1 | ${stats.pwCount}号 D${stats.totalD}·W${stats.totalW} | 监测${_totalMonitorCycles}轮·${_totalChangesDetected}次变动·${_store.switchCount}次切号 | inst=${_instanceId}`;
  ...
  context.subscriptions.push(
    vscode.window.onDidChangeActiveTextEditor(() => { _lastSelfActivity = Date.now(); }),
    vscode.workspace.onDidChangeTextDocument(() => { _lastSelfActivity = Date.now(); }),
    vscode.window.onDidOpenTerminal(() => { _lastSelfActivity = Date.now(); }),
  );
  _lastSelfActivity = Date.now(); // 启动即视为活跃

  // ── 实例协调: 心跳 + 声明 (根治: 多实例抢号) ──
  _writeInstanceClaim(_store.activeIndex >= 0 ? _store.get(_store.activeIndex)?.email : '');
  _heartbeatTimer = setInterval(() => {
    const activeAcc = _store.activeIndex >= 0 ? _store.get(_store.activeIndex) : null;
    _writeInstanceClaim(activeAcc?.email || '');
    _cleanDeadInstances();
  }, INSTANCE_HEARTBEAT_MS);
  context.subscriptions.push({ dispose() { if (_heartbeatTimer) clearInterval(_heartbeatTimer); } });
  log(`instance: ${_instanceId} registered (pid=${process.pid})`);

  // ── 延迟启动: 仅文件监听 (不自动登录, 不自动扫描, 不自动清理) ──
  setTimeout(() => {
    startFileWatcher();
    if (fs.existsSync(TOKEN_FILE) && isWamMode()) {
...
function deactivate() {
  ...
  log(`deactivate — inst=${_instanceId} 监测${_totalMonitorCycles}轮·${_totalChangesDetected}次变动·${_store?.switchCount || 0}次切换 snap=${_quotaSnapshots.size} inUse=${_store?._inUse?.size || 0}`);
}
